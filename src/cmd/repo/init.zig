...
pub fn initCmd(allocator: Allocator, git: *git_mod.Git, homeDir: []const u8, args: []const []const u8) !void {
    ...
    if (args.len > 0) {
        remote_url = std.mem.trim(u8, args[0], " \t\r\n");
    }
    ...
    if (remote_url != null and remote_url.?.len > 0 and (git.fetch() catch null) != null and !git.hasCommits()) {
        ...
        for ([_][]const u8{ "origin/main", "origin/master" }) |b| {
            if (git.output(&[_][]const u8{ "rev-parse", "--verify", b })) |out| {
                allocator.free(out);
        ...
        if (default_branch == null) {
            if (git.output(&[_][]const u8{ "symbolic-ref", "--short", "refs/remotes/origin/HEAD" })) |symref| {
                defer allocator.free(symref);
                if (symref.len > 0) {
                    default_branch = if (std.mem.startsWith(u8, symref, "origin/")) symref["origin/".len..] else symref;
                }
            } else |_| {
                if (git.output(&[_][]const u8{ "rev-parse", "--verify", "FETCH_HEAD" })) |out| {
                    allocator.free(out);
                    default_branch = "main";
                } else |_| {}
            }
        ...
        if (default_branch) |db| {
            ...
            if (git.output(&[_][]const u8{ "rev-parse", "--verify", remote_ref })) |out| {
                allocator.free(out);
                const sym_ref = try std.fmt.allocPrint(allocator, "refs/heads/{s}", .{db});
                defer allocator.free(sym_ref);
                _ = git.run(&[_][]const u8{ "symbolic-ref", "HEAD", sym_ref }) catch {};
                _ = git.run(&[_][]const u8{ "update-ref", sym_ref, remote_ref }) catch {};
